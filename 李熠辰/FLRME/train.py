...
from modules.mm import eval_meld
import transformers
import time
import json
...
#                     print("2")
                    ...
                    batch_FacialEmo_import_matrix = torch.mm(preds_FacialEmo_distr,preds_FacialEmo_distr.t())    
                    batch_FacialEmo_import = torch.diagonal(batch_FacialEmo_import_matrix) 
#                     print("score",batch_FacialEmo_import)
                    batch_FacialEmo_import_mask = batch_FacialEmo_import.gt(args.FacialEmoImpor_threshold)
#                     print(args.FacialEmoImpor_threshold)
                    filtered_scores = batch_FacialEmo_import[batch_FacialEmo_import_mask]
#                     print("Mask (True indicates scores above threshold):", batch_FacialEmo_import_mask.tolist())
#                     print("Filtered Scores (after filtering):", filtered_scores.tolist())
                    batch_HighImporFace_idx = torch.nonzero(batch_FacialEmo_import_mask).squeeze(1)  
                    batch_vis_emo = torch.zeros([vision_inputs.shape[0], vision_inputs.shape[1], args.num_labels])
                    if len(batch_HighImporFace_idx) > 0:
                        temp_batch_HighImporFace_idx = batch_HighImporFace_idx
...
#                         print(logits.shape)
            ...
            ff = os.path.join(args.data_load_path, args.choice_modality, 'path.json')
            with open(ff, "r") as file:
                 data = json.load(file)
            for key, paths in data.items():
                 for path in paths:
                    print(f"images_path {path}")
                    time.sleep(0.02)           
