...
def from_image_to_embedding_no_IncepRes(images_path_list, set_name):
    ...
    for i in range(images_lens):  #
        print("images_path", images_path_list[i][1:])
        im = cv2.imread(images_path_list[i][1:])
...
#         self.text_sep_mask = torch.tensor([f.sep_mask for f in text_inputs], dtype=torch.long).cpu().detach() #(dia_num, max_sequence_len) 
        self.text_input_ids = torch.stack([torch.tensor(f.input_ids[:512], dtype=torch.long) for f in text_inputs])
        self.text_input_mask = torch.stack([torch.tensor(f.input_mask[:512] + [0] * (512 - len(f.input_mask)) if len(f.input_mask) < 512 else f.input_mask[:512], dtype=torch.long) for f in  text_inputs]).cpu().detach()          
        self.text_sep_mask = torch.stack([torch.tensor(f.sep_mask[:512], dtype=torch.long) for f in text_inputs]).cpu().detach()
        ...
        print("set_face_160_path的路径：")
        print(set_face_160_path)         
#         set_face_160_path1 = os.path.join(dataset_path, self.choice_modality, 'val_facseqs_160_paths_final.json')
