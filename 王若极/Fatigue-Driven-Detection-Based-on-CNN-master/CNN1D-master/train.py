...
data = pd.read_csv(r'D:\WorkSpace\Fatigue-Driven-Detection-Based-on-CNN-master\CNN1D-master\light_train.csv')
dataset = data.values
...
data = pd.read_csv(r'D:\WorkSpace\Fatigue-Driven-Detection-Based-on-CNN-master\CNN1D-master\test.csv')
dataset = data.values
...
for i in range(epochs):
    ...
    for data in train_loader:
        ...
        matrix = confusion_matrix(Y_data.cpu().numpy(), pred.cpu().numpy(), labels=[0, 1])
        TN = matrix[0][0]
        FP = matrix[0][1]
        FN = matrix[1][0]
        TP = matrix[1][1]
    ...
    if (i + 1) % 10 == 0:
        ...
        with torch.no_grad():
            for test in test_loader:
                X_test_data, Y_test_data = test[0].to(device), test[1].to(device)
